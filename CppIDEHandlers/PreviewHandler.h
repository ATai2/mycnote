...
#include <afxext.h>
#include "..\CppIDE\CppIDEDoc.h"
#include "..\CppIDE\CppIDEView.h"
...
public:
	virtual HRESULT InitializeDocumentPreview(HWND hWndParent, RECT* prc)
	{
		...
		CCreateContext ctx;
		ctx.m_pNewViewClass = RUNTIME_CLASS(CCppIDEView);
		...
		m_pDocument->AddRef();
		ctx.m_pCurrentDoc = DYNAMIC_DOWNCAST(CCppIDEDoc, (CObject*) m_pDocument->GetContainer());
