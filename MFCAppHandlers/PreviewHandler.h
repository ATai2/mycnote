...
#include <afxext.h>
#include "..\MFCApp\MFCAppDoc.h"
#include "..\MFCApp\MFCAppView.h"
...
public:
	virtual HRESULT InitializeDocumentPreview(HWND hWndParent, RECT* prc)
	{
		...
		CCreateContext ctx;
		ctx.m_pNewViewClass = RUNTIME_CLASS(CMFCAppView);
		...
		m_pDocument->AddRef();
		ctx.m_pCurrentDoc = DYNAMIC_DOWNCAST(CMFCAppDoc, (CObject*) m_pDocument->GetContainer());
